    ...
    public int add(Connection connection, Provider provider)
        ...
        if (null != connection) {
            String sql = "insert into smbms_provider (proCode,proName,proDesc," +
                    "proContact,proPhone,proAddress,proFax,createdBy,creationDate,idPicPath,orgCodePicPath) " +
                    "values(?,?,?,?,?,?,?,?,?,?,?)";
            Object[] params = {provider.getProCode(), provider.getProName(), provider.getProDesc(),
                    ...
                    provider.getProFax(), provider.getCreatedBy(), provider.getCreationDate(), provider.getIdPicPath(), provider.getOrgCodePicPath()};
